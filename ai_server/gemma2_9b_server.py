#!/usr/bin/env python3
"""
Reality Lab Gemma2-9B Server - Multi-GPU Load Balancing
...
def load_model_on_gpu(gpu_id):
    """Load Gemma2-9B model on a specific GPU"""
    try:
        device = torch.device(f"cuda:{gpu_id}")
        model_name = "EleutherAI/gpt-j-6b"
        
        logger.info(f"Loading Gemma2-9B model on GPU {gpu_id}")
        
        # Set the current GPU
        torch.cuda.set_device(gpu_id)
        
        # Load model with token (try without token first, then with token if needed)
        try:
            ...
            logger.warning(f"Failed without token, trying with token: {e}")
            # Try with common environment variable names for HF token
            import os
...
def load_all_models():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Tokenizer failed without token, trying with token: {e}")
            import os
            token = os.environ.get('HF_TOKEN') or os.environ.get('HUGGING_FACE_HUB_TOKEN')
            if token:
        ...
        # Load models on each GPU (only on GPU 0 and 1 due to size)
        target_gpus = [0, 1]  # Gemma2-9B is large, use only 2 GPUs
    ...
    try:
        ...
        with gpu_locks[best_gpu]:
            ...
            messages = [
                {
                    "role": "user", 
                    "content": f"""당신은 숭실대학교 Reality Lab의 전문 어시스턴트입니다. 연구실에 대한 질문에 정확하고 간결하게 답변하세요.
...
if __name__ == '__main__':
    logger.info("Starting Multi-GPU Reality Lab Gemma2-9B Server...")
    
    # Load models on all GPUs
    if load_all_models():
        logger.info("🚀 Multi-GPU Gemma2-9B server ready!")
