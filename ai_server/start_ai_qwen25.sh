...
# AI Server Auto-Start Script with Qwen2.5-3B
...
echo "[$(date)] Starting AI server with Qwen2.5-3B and smart GPU selection..."
...
# Start new server with selected GPUs and Qwen2.5-3B model
echo "[$(date)] Starting AI server with Qwen2.5-3B on GPUs: $GPU_LIST"
...
echo "[$(date)] Model: Qwen2.5-3B (Faster)"
...
if ps -p $NEW_PID > /dev/null 2>&1; then
    echo "[$(date)] ✅ AI server (Qwen2.5-3B) is running successfully!"
    exit 0
else
